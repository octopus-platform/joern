...
public class ReverseCFG<V> implements Graph<V>
{
	...
	public List<V> outNeighborhood(V block)
	{
		if (!contains(block))
		{
			throw new IllegalArgumentException("Graph has no such vertex "
					+ block);
		}
	...
	public int outDegree(V block)
	{
		...
		return outNeighborhood.containsKey(block) ? outNeighborhood.get(block)
				.size() : 0;
	}
	...
	public int inDegree(V block)
	{
		...
		return inNeighborhood.containsKey(block) ? inNeighborhood.get(block)
				.size() : 0;
	}
	...
	public static ReverseCFG<Object> newInstance(DefUseCFG cfg, Object start,
			Object exit)
	{
	...
	private static class ReverseCFGCreator<V>
	{
		...
		private void linkBlocks()
		{
			...
			MultiHashMap<V, V> outNeighborhood = (MultiHashMap<V, V>) cfg
					.getChildBlocks();
			MultiHashMap<V, V> inNeighborhood = (MultiHashMap<V, V>) cfg
					.getParentBlocks();
